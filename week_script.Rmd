...
participant_data <- list.files(path = "data/raw", pattern = "actigraphy.csv$") %>% 
  set_names()
...
```{r}
# get the csv files in a named vector
participant_data <- list.files(path = "data/raw", pattern = "PANAS.csv$") %>% 
  set_names()
...
write_csv(tRMSSD_calculations, "data/processed/tRMSSD_calculation.csv")
```
