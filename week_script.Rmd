...
write_csv(tRMSSD_actigraphy_participant_overall, "data/processed/tRMSSD_panas_by_participant.csv")
```
...
```{r}
condition_info_panas <- read_csv("data/COMET_group_panas.csv") %>% 
  select(-`Participant ID`) %>% 
  mutate(Group = if_else(Group == 2, "low MDQ", "high MDQ")) %>% 
  rename(participant_id = ID)
