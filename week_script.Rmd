...
First we need to give them a study week number - so for all participants, we say that their first week is week 1, and then we just count from there. So everyone's scores a within a period of 10 weeks.

```{r}
tRMSSD_acti_weeks <- tRMSSD_acti_w_group %>% 
  group_by(participant_id) %>% 
  mutate(study_week = week - min(week) + 1) %>%
  # filter(study_week <= 10) %>% 
  relocate(study_week)

tRMSSD_acti_weeks %>% 
  arrange(participant_id, week) %>% view()
```
...
tRMSSD_acti_weeks %>% 
  ...
  ggplot() +
    aes(x = study_week, y = tRMSSD_RA, color = Group) +
...
tRMSSD_acti_long <- tRMSSD_acti_weeks %>% 
  select(-week) %>% 
  relocate(participant_id, Group, study_week) %>% 
  pivot_longer(-c(participant_id, Group, study_week)) %>% 
  rename(measure = name) %>% 
...
tRMSSD_acti_long %>% 
  ggplot() +
    aes(x = study_week, y = value, color = Group) +
    geom_jitter(height = 0, width = 0.1, alpha = 0.5) +
    geom_line(aes(group = participant_id)) +
...
tRMSSD_aves <- tRMSSD_acti_long %>% 
  group_by(Group, study_week, measure) %>% 
  summarise(ave_tRMSSD = mean(value, na.rm = TRUE))
  

tRMSSD_aves %>% 
  ggplot() +
    aes(x = study_week, y = ave_tRMSSD, color = Group) +
...
tRMSSD_acti_weeks %>% 
  mutate(study_week = factor(study_week)) %>% 
  ggplot() +
    aes(x = study_week, y = tRMSSD_RA, color = participant_id) +
    geom_jitter(height = 0, width = 0.1, alpha = 0.5) +
    geom_line(aes(group = participant_id)) +
    facet_wrap(~Group)
```
...
```{r, message=FALSE}
# get the csv files in a named vector
participant_data <- list.files(path = "data/raw/PANAS", pattern = "PANAS.csv$", full.names = TRUE) %>% 
  set_names()
